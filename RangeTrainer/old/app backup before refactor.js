...
// Helper to manage mistake tracking
const MistakeTracker = {
    getFails: () => JSON.parse(localStorage.getItem('pokerFails') || '{}'),

    recordFail: (rangeName, handIndex) => {
        const fails = MistakeTracker.getFails();
        const id = `${rangeName}_${handIndex}`;
        fails[id] = (fails[id] || 0) + 1; // Increment fail count
        localStorage.setItem('pokerFails', JSON.stringify(fails));
    },

    recordSuccess: (rangeName, handIndex) => {
        const fails = MistakeTracker.getFails();
        const id = `${rangeName}_${handIndex}`;
        if (fails[id]) {
            fails[id] = Math.max(0, fails[id] - 1); // Reduce weight on success
            if (fails[id] === 0) delete fails[id];
            localStorage.setItem('pokerFails', JSON.stringify(fails));
        }
    }
};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    //Load a new quiz
    // Helper to manage mistake tracking via localStorage
    const MistakeTracker = {
        getFails: () => JSON.parse(localStorage.getItem('pokerFails') || '{}'),
        ...
        recordSuccess: (rangeName, handIndex) => {
            ...
            const id = `${rangeName}_${handIndex}`;
            if (fails[id]) {
                fails[id] = Math.max(0, fails[id] - 1);
                if (fails[id] === 0) delete fails[id];
                localStorage.setItem('pokerFails', JSON.stringify(fails));
            }
        }
    ...
    function loadNewQuiz() {
        ...
        // 1. Check which Quiz Types are currently enabled
        ...
        if (validQuizTypes.length === 0) {
            printText('Please select at least one quiz type.');
            return;
        }

        // 2. Determine if we should pick a "Review" hand (Spaced Repetition)
        const fails = MistakeTracker.getFails();
        const failIds = Object.keys(fails);
        let randomRow;
        let randomHandIndex;
        let isReviewHand = false;
        console.log("FailIDs length: " + failIds.length);
        // 40% chance to pick a failed hand if any exist and match filters
        if (failIds.length > 0 && Math.random() < 0.4) {
            console.log("failed hand");
            const randomFailId = failIds[Math.floor(Math.random() * failIds.length)];
            const [rangeName, handIdx] = randomFailId.split('_');

            const rowFound = rows.find(r => r[headers.indexOf('RangeName')] === rangeName);
            if (rowFound) {
                const quizType = rowFound[headers.indexOf("QuizType")];
                if (validQuizTypes.includes(quizType)) {
                    randomRow = rowFound;
                    randomHandIndex = parseInt(handIdx);
                    isReviewHand = true;
                }
            }
        }
        ...
        if (!isReviewHand) {
            let rowIdx = Math.floor(Math.random() * (rows.length - 1)) + 1;
            randomRow = rows[rowIdx];
            let quizType = randomRow[headers.indexOf("QuizType")];

            while (!validQuizTypes.includes(quizType)) {
                rowIdx = Math.floor(Math.random() * (rows.length - 1)) + 1;
                randomRow = rows[rowIdx];
        ...
        // 4. Process the selected Hand and Range
        ...
        let index = 1;
        while (randomRow[headers.indexOf("Action" + index)] != "" &&
            randomRow[headers.indexOf("Action" + index)] != undefined &&
            randomRow[headers.indexOf("Range" + index)] != "") {
        ...
        // 5. Update global question state
        question = {
            ...
            currentHandIndex: randomHandIndex, // ID for mistake tracker
            currentRangeName: randomRow[headers.indexOf('RangeName')], // ID for mistake tracker
            isReview: isReviewHand
        };

        popupCurrentHand.ranges = popupRanges;
        popupCurrentHand.actions = popupActions;

        // 6. Setup the Visual Poker Table
        ...
        // Rotate positions so Hero is always at the bottom-center
        ...
        // Visual indicator for review hands
        if (isReviewHand) {
            printText("Reviewing a hand you've struggled with...");
        }
    }
    ...
    function checkIfCorrectAction(clickedButton) {
        ...
        if (clickedButton.dataset.isCorrect === "true") {
            ...
            MistakeTracker.recordSuccess(question.currentRangeName, question.currentHandIndex);
            loadNewQuiz();
        } else {
            feedback.innerHTML = '<span style="color: Red;">Wrong!</span> ' + htmlString + '(RNG: ' + RNGvalue + ") <br>";
            feedback.innerHTML += '<br>Global frequency: ' + htmlString2;
            MistakeTracker.recordFail(question.currentRangeName, question.currentHandIndex);
        }
    }
