...
const labelPositions = [
    { x: 300, y: 60, chipx: 0, chipy: 80 },
    { x: 520, y: 90, chipx: -40, chipy: 80 },
    { x: 520, y: 370, chipx: 10, chipy: -75 },
    { x: 300, y: 420, chipx: -70, chipy: -50 },
    { x: 80, y: 370, chipx: -10, chipy: -75 },
    { x: 80, y: 90, chipx: 40, chipy: 80 }
];
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function drawHerohand(handString, dx) {
        ...
        // Draw rank
        ctx.fillText(rank, 260 + dx + 36 / 2, 340 + 50 / 3);

        // Draw suit
        ctx.fillText(suitsSymbols[suit], 260 + dx + 36 / 2, 340 + (2 * 50) / 3);
    ...
    function drawTable(positions, invested, holdingCards, board) {
        ...
        ctx.beginPath();
        ctx.ellipse(300, 230, 280, 180, 0, 0, Math.PI * 2);
    ...
    function drawButtons(actionChoice) {
        ...
        for (let index = 0; index < actionOptions.length; index++) {
            ...
            btn.addEventListener('click', () => checkIfCorrectAction(actionOptions[index]));
    ...
    function checkIfCorrectAction(frequencyGuess) {
        ...
        if (frequencyGuess === correctFreqBucket1 && correctFreqBucket2 == null) {
            ...
            loadNewQuiz();
        } else if (frequencyGuess === correctFreqBucket1 && correctFreqBucket2 != null) {
            feedback.textContent = 'Correct! Nice job! Frequency was ' + parseFloat(correctFrequency1) * 100.0 + "%";
            correctFreqBucket1 = null;
            if (correctFrequency1 == 1) {
                loadNewQuiz();
            }
            drawButtons(action2);
        } else if (correctFreqBucket1 === null && frequencyGuess == correctFreqBucket2) {
            feedback.textContent = 'Correct2! Nice job! Frequency was ' + parseFloat(correctFrequency2) * 100.0 + "%";
            loadNewQuiz();
        } else if (correctFreqBucket1 === null) {
            feedback.textContent = 'Incorrect. Correct answer: ' + parseFloat(correctFrequency2) * 100.0 + "%";
        } else {
            feedback.textContent = 'Incorrect. Correct answer: ' + parseFloat(correctFrequency1) * 100.0 + "%";
        }
    }
