 ...
 * @param {function(Event): void} onClickHandler - The function to call when a cell is clicked.
 * @param {Array<string>} [content] - Optional array of strings to populate the cells.
 */
...
/**
 * The main click handler for the hand matrix cells.
...
document.addEventListener('DOMContentLoaded', () => {
    const mainMatrix = document.getElementById('main-matrix');
    const smallMatrix1 = document.getElementById('matrix-2bet');
    const smallMatrix2 = document.getElementById('matrix-3bet');
    ...
    for (let i = 0; i < positions.length; i++) {
        for (let j = 0; j < positions.length; j++) {
            if (j % 6 == 0) {
                positionsVsPositions.push(`v ${positions[i]}`);
                positionsVsPositions2.push(`h ${positions[i]}`);
            } else if (j > i) {
                positionsVsPositions.push(`${positions[j]}`);
                positionsVsPositions2.push(`${positions[j]}`);
            } else {
                positionsVsPositions.push(` `);
                positionsVsPositions2.push(` `);
            }
        }
    }
