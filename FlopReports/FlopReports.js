    ...
    data.forEach((row) => {
        ...
        if (drawRow) {
            ...
            while (nextX < x + colWidth) {
                canvas.tooltipData[nextX] = row.flop + "<br>Check: " + row.Check + "%<br>B33: " + row.B33 + "%<br>B66: " + row.B66 + "%<br>B130: " + row.B130 + "%<br>All In: " + row.AI + "%";
                nextX++;
            }
...
function addHoverEffect(data, canvas) {
    ...
    canvas.addEventListener('mousemove', function (event) {
        ...
        if (index >= 0 && index < data.length) {
            ...
            tooltip.innerHTML = `Flop: ${canvas.tooltipData[x]}`;
        } else {
