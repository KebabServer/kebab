...
document.querySelectorAll('.hand-matrix').forEach(handMatrix => {
    ...
    ranges = handMatrix.getAttribute('range-freqs').split("*");
    var colorsf_multi = [[[], [], [], [], [], [], [], [], [], [], [], [], []],
    [[], [], [], [], [], [], [], [], [], [], [], [], []],
    [[], [], [], [], [], [], [], [], [], [], [], [], []],
    [[], [], [], [], [], [], [], [], [], [], [], [], []],
    ...
    const cardString = "23456789TJQKA";
    let ind = 0;
    for (let x = 0; x < 13; x++) {
        for (let y = 0; y < 13; y++) {
    ...
    for (let n = 0; n < colors.length; n++) {
        for (let x = 0; x < 13; x++) {
            for (let y = 0; y < 13; y++) {
                colorsf_multi_tooltip[x][y] += "<div style='color:" + colors[n] + "; font-weight: bold;'>" + colorsf_multi_tooltip_temp[x][y] + "</div>";
    ...
    for (let i = 0; i < 13; i++) {
        ...
        for (let j = 0; j < 13; j++) {
            ...
            var str = "";
            for (let n = 0; n < colors.length; n++) {
                str += colors[n] + " " + x + "% " + (x + colorsf_multi[12 - i][12 - j][n] / heightsMax[i][j]) + "%,";
                x += colorsf_multi[12 - i][12 - j][n] / heightsMax[i][j];
            }
            tempDiv.style.backgroundImage = "linear-gradient(to right, " + str + " gray " + x + "% 100%)";
            tempDiv.setAttribute("tooltipText", colorsf_multi_tooltip[12 - i][12 - j]);
